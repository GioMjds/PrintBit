...
export function calculateJobAmount(
  mode: PrintMode,
  colorMode: ColorMode,
  copies: number,
): number {
...
export async function appendAdminLog(
  ...
  if (db.data!.logs.length > MAX_LOGS) {
    db.data!.logs.length = MAX_LOGS;
  }
  await db.write();
  return entry;
}

export async function incrementCoinStats(coinValue: number): Promise<void> {
...
export function computeEarningsBuckets(now = new Date()) {
...
export function getStorageUsage(uploadDir: string) {
...
export function logsToCsv(logs: AdminLogEntry[]): string {
